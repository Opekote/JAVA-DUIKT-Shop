        ...
        while (true) {
            ...
            switch (option) {
                ...
                case 3 -> {
                    ...
                    try {
                        System.out.println(storeInstance.getProductByName(productName));
                    } catch (ProductNotFoundException ex) {
                        System.err.println(ex.getMessage());
                    }
                }
                case 4 -> {
                    ...
                    String productName = scanner.next();
                    try {
                        Product product = storeInstance.getProductByName(productName);
                        cart.addProduct(product);
                        System.out.println("Товар додано до кошика: " + productName);
                    } catch (ProductNotFoundException ex) {
                        System.err.println(ex.getMessage());
                    }
                }
                ...
                case 7 -> {
                    System.out.println("Введіть назву товару для вилучення з кошика");
                    String productName = scanner.next();
                    try {
                        Product product = storeInstance.getProductByName(productName);
                        if (cart.getProducts().contains(product)) {
                            cart.removeProduct(product);
                            System.out.println("Товар вилучено з кошика: " + productName);
                        } else {
